...
#        return env.find(x.s)[x.s]
#    elif x.t != "list":         # constant literal
#        return x               
...
#        (_, var, exp) = xlist
#        env[var.s] = eval(exp, env)
#    elif xcar == 'lambda':         # (lambda (var*) exp)
#        (_, vars, exp) = xlist
#        return atom(lambda *args: eval(exp, Env(vars, args, env)))
#    elif xcar == 'begin':          # (begin exp*)
#        for exp in xlist[1:]:
#            val = eval(exp, env)
#        return atom(val)
#    else:                          # (proc exp*)
#        exps = [eval(exp, env) for exp in xlist]
#        proc = exps.pop(0)
#        return atom(proc(*exps))
